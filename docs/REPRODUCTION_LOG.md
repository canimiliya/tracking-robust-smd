...
## S3-R0 Closed-Loop Quadrotor Dynamics Bridge

- TASK_ID: `S3-R0-CLOSED-LOOP-QUADROTOR-DYNAMICS-BRIDGE-R1`
- S2 was integrated into `main` by fast-forward at `849d1fa2de5c564ad86caaba050319c35297986b`; S3 work is isolated on `feat/closed-loop-dynamics`.
- No SMD planner rerun was performed. The accepted `instances_simple`, `instance_idx=0`, 3-agent, candidate-0 raw path was reused with SHA-256 `76024E724A2F51E4891B6EEC602E2DA060DEAD504AC23BC5C89D6F9020BB53FB`.
- Reference timing is frozen at `trajectory_duration=5.0 s`, `n_support_points=64`, `dt_ref=0.078125 s`, and support span `4.921875 s`. Knot parity passed exactly: position `0.0 m`, velocity `0.0` in the recorded floating-point comparison. Raw-velocity consistency was retained as an audit only: RMSE `0.15395999853895792`, max error `0.3510340690612793`.
- The CF2X provenance is recorded in `docs/CLOSED_LOOP_DYNAMICS_CONTRACT.md` and `experiments/manifests/s3_r0_closed_loop_manifest.json`; the model is a 13-state nonlinear rigid body with fixed-step RK4 at `0.002 s`, 100 Hz control, clipped rotor allocation, and XY execution collision checks at every dynamics step.
- Physical unit tests passed manually because `smd-blackwell` does not contain `pytest`: free fall, hover wrench, allocation rank/sign/reconstruction, quaternion normalization, reference timing/knot parity, and controller finiteness/bounds.
- Synthetic gain audit used only hover and independent `+0.10 m x` / `+0.10 m y` setpoint tests. Revisions 1, 2, and 3 were finite and stable; revision 3 is frozen. Both setpoint tests settled below `0.02 m` at `0.846 s`, with final error below `6e-7 m`.
- The formal 3-agent calm SMD smoke was executed through actual RK4 state propagation. It was execution-geometrically collision-free and finite, but the frozen tracking gate failed: mean XY RMSE `0.027994756572505247 m` (limit `0.02`), maximum XY error `0.10764344523313567 m` (limit `0.05`), and Z RMSE `0.02453234169277586 m` (limit `0.01`). Per-agent final XY errors passed; rotor saturation fraction `0.0064974010395841665` and max tilt `30.67673502511937 deg` passed.
- Per the task card, no further gain tuning, trajectory modification, planner change, artificial position reset, wind, robust-planning, or S4 work was performed. This stage is `BLOCKED`, with raw artifact `experiments/raw/s3_r0_closed_loop_3agent.npz`, summary `experiments/summaries/s3_r0_closed_loop_summary.json`, and three figures under `experiments/figures/`.

## S3-R1 Projected Reference Derivative-Consistency Repair

- TASK_ID: `S3-R1-PROJECTED-REFERENCE-DERIVATIVE-CONSISTENCY-REPAIR-R1`; branch `feat/closed-loop-dynamics`; the frozen S3-R0 controller, dynamics, timing, limits, and source path were retained.
- Official source audit confirmed that `smd/projection/projection.py` copies `x_candidate` and overwrites only planar position dimensions; it does not recompute the retained velocity state. The nine accepted S1 raw runs therefore show systemic position/velocity inconsistency. Evidence: `docs/S3_REFERENCE_SEMANTICS_AUDIT.md` and `experiments/summaries/s3_r1_velocity_semantics_audit.csv`.
- S3-R1 uses projected SMD position as physical geometry authority, position-derived central-difference velocities with zero endpoints, analytic Hermite acceleration, and an explicit final hold with `v_ref=a_ref=0`. Support points were not changed.
- Reference audits passed: position knot error `0.0 m`, `dp/dt-v` max error `1.1626120205443513e-10`, `dv/dt-a` max error `1.7032662502636242e-09`, nominal inter-agent clearance `0.0862374644468745 m`, nominal obstacle clearance `0.024229648852440708 m`, polyline deviation `0.0021453417908650373 m`, and required thrust ratio `0.6813235342782609`.
- Controller config SHA-256 was unchanged before/after (`38F8C62CD39A86A49995DD110B16141AE8B285102332578F11D155A3E9451677`), revision count remained `3`, and synthetic hover / `+0.10 m x` / `+0.10 m y` regression passed after restoring the original off-reference synthetic initial state.
- The only formal rerun reused `instances_simple`, `instance_idx=0`, 3 agents, candidate 0, with paths SHA-256 `76024E724A2F51E4891B6EEC602E2DA060DEAD504AC23BC5C89D6F9020BB53FB`. Actual initial position and velocity mismatches were both `0.0`.
- Frozen S3 tracking gate passed: mean XY RMSE `0.004417622834878108 m`, maximum XY error `0.026932576189519285 m`, Z RMSE `0.007891901734266044 m`, max tilt `27.692595762720078 deg`, rotor saturation `0.0`, and strict execution collision-free. R0/R1 comparison is in `experiments/summaries/s3_r0_vs_r1_tracking_comparison.csv` and reference demand comparison in `experiments/summaries/s3_r1_reference_comparison.json`.
- Raw artifact: `experiments/raw/s3_r1_closed_loop_3agent.npz`, SHA-256 `7D06681D126CE615A68E109C68440414008E73195695BF767498DC179F3010B7`; four new figures are under `experiments/figures/`. Root-cause evidence is in `docs/S3_R0_ROOT_CAUSE.md`.
- This remediation is `SUBMITTED_FOR_REVIEW` with final label `S3_R1_PROJECTED_REFERENCE_CONSISTENCY_READY`. S3 is not independently marked `CLOSED`, formal progress is not advanced here, and S4 remains frozen pending `HIGH_LEVEL_S3_CLOSURE_REVIEW`.
